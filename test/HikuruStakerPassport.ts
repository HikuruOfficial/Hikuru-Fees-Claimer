...
const hikuruPiggyBankAddss = "0x2279B7A0a67DB372996a5FaB50D91eAA73d2eBe6";
const hikuruFeeClaimerAddss = "0x8A791620dd6260079BF849Dc5567aDC3F2FdC318";
...
// }; isWhitelisted: (arg0: any, arg1: number) => any; uri: (arg0: number) => any; };
    ...
    let feeClaimer: {
        ...
        getBalance(address: any): any; claimFees: (arg0: any, arg1: any) => any; 
    };
    ...
    beforeEach(async function () {
        ...
        HikuruBadges = await ethers.getContractFactory("contracts/HikuruStakerPassport.sol:HikuruStakerPassport");
        hikuruBadges = await HikuruBadges.deploy(owner.address, hikuruPiggyBankAddss, hikuruFeeClaimerAddss, "default_uri");

        // import FeeClaimer from ABI on spesific contract addres
        feeClaimer = new ethers.Contract(hikuruFeeClaimerAddss, feeClaimerAbi, owner);
    });
...
describe("Minting with Referral", function () {
    ...
    it("Should transfer referral fee and mint fee correctly", async function () {
        const initialReferralBalance = await feeClaimer.getBalance(referral.address);
        const initialNotReferralBalance = await feeClaimer.getBalance(owner.address);


        const initialBalanceOfFeeClaimer = await ethers.provider.getBalance(feeClaimer.target);
        ...
        const finalReferralBalance = await feeClaimer.getBalance(referral.address);
        const finalNotReferralBalance = await feeClaimer.getBalance(owner.address);
        const finalBalanceOfFeeClaimer = await ethers.provider.getBalance(feeClaimer.target);
        ...
        expect(finalNotReferralBalance).to.equal(initialNotReferralBalance);
        expect(finalBalanceOfFeeClaimer).to.equal(initialBalanceOfFeeClaimer+mintingFeeForRef);
